...
func update_spawn_node() -> void:
	...
	ingredient_spawn_position = spawn_node.position
...
func _take_out(ingredient_name: String, amount: int) -> void:
	...
	print(ingredient_instance.mass_unit)
	ingredient_instance.amount = amount * ingredient_instance.mass_unit
	ingredient_instance.mass = ingredient_instance.amount / 1000.
	ingredient_instance.global_position = ingredient_spawn_position
	get_tree().current_scene.add_child(ingredient_instance)
	ingredient_instance.container.update_effect_display()
