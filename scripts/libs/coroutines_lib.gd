...
static func invoke_repeating(function: Callable, tree: SceneTree, timers_node: Node, repeat_time: int=1, offset: int=- 1, max_times=0) -> void:
	var function_hash := hash(function)
	assert(function_hash not in _coroutines, "Function %s is already being called" % function)
	...
	_coroutines[function_hash] = timer
	...
	while function_hash in _coroutines and not timer.is_stopped() and (max_times <= 0 or called_times < max_times):
		await timer.timeout
		if function_hash not in _coroutines:
			break
		function.call()
		called_times += 1
